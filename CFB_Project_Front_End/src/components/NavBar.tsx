...
const NavBar = () => {
    ...
    const isActive = (path: string) => {
        ...
        return currentPath === path || (path.startsWith("/myaccount") && currentPath.startsWith("/myaccount"));
    };
                    ...
                    <div className="flex items-center space-x-8">
                        ...
                        <div className="hidden sm:ml-6 sm:flex sm:space-x-8">
                            <Link to={"/"}><p className={`${isActive("/") ? "text-white border-b-2 border-white" : "text-gray-500 border-transparent"
                                }"text-white hover:text-gray-700 inline-flex items-center px-1 pt-1 text-sm font-medium`}>Home</p></Link>
                            <Link to={"myaccount/personal_details"}><p className={`${isActive("/myaccount") ? "text-white border-b-2 border-white" : "text-gray-500 border-transparent"
                                }"text-white hover:text-gray-700 inline-flex items-center px-1 pt-1 text-sm font-medium`}>My Accounts</p></Link>
                            <Link to={"aboutus"}><p className={`${isActive("/aboutus") ? "text-white border-b-2 border-white" : "text-gray-500 border-transparent"
                                }"text-white hover:text-gray-700 inline-flex items-center px-1 pt-1 text-sm font-medium`}>Learn</p></Link>
                        </div>
                    </div>
                        <div className="hidden sm:ml-6 sm:flex sm:space-x-4">
                            <Link to={"/contactus"}><p className={`${isActive("/contactus") ? "text-white border-b-2 border-white" : "text-gray-500 border-transparent"
                                }"text-white hover:text-gray-700 inline-flex items-center px-1 pt-1 text-sm font-medium`}>Contact Us</p></Link>
                            {user ? (
                                ...
                                <Link to={"/login"}><BiUser className="h-6 w-6 mr-1 inline-flex image-center" style={{ fill: '#3B8CD5', stroke: '#3B8CD5', strokeWidth: 1 }}/>
                                    <p className={`${isActive("/login") ? "text-white border-b-2 border-white" : "text-gray-500 border-transparent"
                                        }"text-white hover:text-gray-700 inline-flex items-center px-1 pt-1 text-sm font-medium`}>
                                        Login</p>
                                </Link>
                            )}
                        </div> 
